                    ...
                    @foreach($invoices as $invoice)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <br><small style="color:#999;">@if($item->sku)SKU: {{ $item->sku }} @endif @if($invoice->invoice_number) | Inv: {{ $invoice->invoice_number }} @endif @if($invoice->repair_id) | Ref: {{ $invoice->repair?->reference ?: ('#'.($invoice->repair?->repair_number ?? $invoice->repair_id)) }} @elseif($invoice->order_id) | Ref: {{ $invoice->order?->order_number ?? '#'.$invoice->order_id }} @endif</small>
                            </td>
