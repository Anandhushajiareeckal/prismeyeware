        ...
        <form action="{{ route('orders.update', $order) }}" method="POST" id="orderForm">
            ...
            <div class="row g-3 mb-4 border-bottom pb-4">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control bg-light border-0 fw-medium text-dark" value="{{ $order->customer->full_name ?? 'Unknown' }}" readonly>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label fw-medium text-muted">Order Date <span class="text-danger">*</span></label>
                    <input type="date" name="order_date" class="form-control bg-light border-0" value="{{ old('order_date', \Carbon\Carbon::parse($order->order_date)->format('Y-m-d')) }}" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-medium text-muted">Sales Staff</label>
                    <input type="text" name="sales_staff" class="form-control bg-light border-0" value="{{ old('sales_staff', $order->sales_staff) }}">
                </div>
                <div class="col-md-6">
            ...
            <div class="table-responsive mb-4">
                <table class="table table-bordered align-middle" id="itemsTable">
                    ...
                    <tbody id="itemsBody">
                        ...
                        <tr class="item-row">
                            <td><input type="text" name="items[{{ $index }}][product_name]" class="form-control bg-light border-0 product-name" required value="{{ old('items.'.$index.'.product_name', $item->product_name) }}"></td>
                            <td>
                                <select name="items[{{ $index }}][category]" class="form-select bg-light border-0 category">
                                    <option value="Frames" {{ $item->category == 'Frames' ? 'selected' : '' }}>Frames</option>
                                    <option value="Lenses" {{ $item->category == 'Lenses' ? 'selected' : '' }}>Lenses</option>
                                    <option value="Contact Lenses" {{ $item->category == 'Contact Lenses' ? 'selected' : '' }}>Contact Lenses</option>
                            ...
                            <td><input type="number" step="0.01" name="items[{{ $index }}][unit_price]" class="form-control bg-light border-0 text-end price fw-medium" value="{{ old('items.'.$index.'.unit_price', $item->unit_price) }}" min="0" required></td>
            ...
            <div class="text-end pt-3 mt-4 border-top">
                <a href="{{ route('orders.show', $order) }}" class="btn btn-light me-2 px-4">Cancel</a>
                <button type="submit" class="btn btn-primary px-4 shadow-sm">Update Order</button>
            </div>
        </form>
    </div>
</div>
